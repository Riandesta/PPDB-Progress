...
<form action="{{ isset($pendaftaran) ? route('pendaftaran.update', $pendaftaran->id) : route('pendaftaran.store') }}" method="POST">
    @csrf
    ...
    @endif
        ...
        <div class="mb-3">
            ...
            <input type="date" class="form-control @error('tgl_lahir') is-invalid @enderror" id="tgl_lahir"
                name="tgl_lahir" value="{{ old('tgl_lahir', $calonSiswa->tgl_lahir ?? '') }}" required>
            @error('tgl_lahir')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <div class="form-check">
                <input type="radio" class="form-check-input" name="jenis_kelamin" value="P"
                    {{ old('jenis_kelamin', $calonSiswa->jenis_kelamin ?? '') == 'P' ? 'checked' : '' }} required>
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control @error('alamat') is-invalid @enderror" id="alamat" name="alamat"
                      rows="3" required>{{ old('alamat', $calonSiswa->alamat ?? '') }}</textarea>
            @error('alamat')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('nama_ortu') is-invalid @enderror"
                   ...
                   value="{{ old('nama_ortu', $calonSiswa->nama_ortu ?? '') }}" required>
            @error('nama_ortu')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('no_telp_ortu') is-invalid @enderror"
                   ...
                   value="{{ old('no_telp_ortu', $calonSiswa->no_telp_ortu ?? '') }}" required>
            @error('no_telp_ortu')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            @if(isset($calonSiswa) && $calonSiswa->foto)
                <div class="mb-2">
                    <img src="{{ asset('storage/'.$calonSiswa->foto) }}" alt="Foto Siswa" class="img-thumbnail" style="max-width: 200px">
                </div>
            @endif
        ...
        <div class="d-flex justify-content-between align-items-center mt-4">
            <a href="{{ route('pendaftaran.show') }}" class="btn btn-secondary">
                <i class="fas fa-arrow-left"></i> Kembali
            </a>
            <button type="submit" class="btn btn-primary">
                <i class="fas fa-save"></i> {{ isset($calonSiswa->id) ? 'Update' : 'Simpan' }}
            </button>
        </div>
    </form>
